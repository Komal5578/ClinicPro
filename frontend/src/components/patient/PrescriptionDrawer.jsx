...
const PrescriptionDrawer = ({ onClose }) => {
  ...
  const getPdfUrlForConsultation = (consultationId) => {
    const prescription = history?.prescriptions?.find(
      (item) => Number(item.consultation_id) === Number(consultationId),
    );

    return prescription?.pdf_path
      ? `${apiOrigin}/pdfs/${prescription.pdf_path}`
      : "";
  };
    ...
    try {
      ...
      try {
        const historyRes = await fetch(
          `${apiOrigin}/api/consultations/patient/${patientData.patient_id}`,
                  ...
                  return (
                    ...
                    >
                      ...
                      {c.followup_date && (
                        ...
                        >
                          Follow-up:{" "}
                          {new Date(c.followup_date).toLocaleDateString(
                            "en-IN",
                          )}
                        </div>
                      )}
