...
import { searchPatient, registerPatient, registerWalkIn, getTodayWalkIns, updateWalkInStatus } from '../../services/api';
...
const WalkInRegister = () => {
  ...
  const [newPatient, setNewPatient] = useState({ name: '', age: '', phone: '', email: '' });
  const [walkInForm, setWalkInForm] = useState({ priority: 'REGULAR', chief_complaint: '' });
  const [walkIns, setWalkIns] = useState([]);
  const [step, setStep] = useState('search'); // search | register | walkin | done
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setPatient({ patient_id: res.data.patient_id, ...newPatient, name: newPatient.name });
      setStep('walkin');
    } catch (err) {
        ...
        <div className="grid-2" style={{ gap: 28, alignItems: 'start' }}>
            ...
            <div className="card">
                ...
                <form onSubmit={handleWalkIn}>
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14.5 }}>{patient.name}</div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {[
                        ...
                        <button
                          key={p.val}
                          type="button"
                          onClick={() => setWalkInForm(f => ({ ...f, priority: p.val }))}
                          style={{
                            flex: 1, padding: '8px 10px', borderRadius: 9,
                            border: `1.5px solid ${walkInForm.priority === p.val ? p.color : '#e2e8f0'}`,
                            background: walkInForm.priority === p.val ? p.bg : 'white',
                            color: walkInForm.priority === p.val ? p.color : '#64748b',
                  ...
                  <div className="form-group">
                    <label className="form-label">Chief Complaint <span style={{ color: '#94a3b8', fontWeight: 400 }}>(optional)</span></label>
                    <input
                      className="form-input"
                      placeholder="e.g. Fever and headache since 2 days"
                      value={walkInForm.chief_complaint}
                      onChange={e => setWalkInForm(f => ({ ...f, chief_complaint: e.target.value }))}
                    />
                  </div>
              ...
              {step === 'done' && (
                <div style={{ textAlign: 'center', padding: '12px 0' }}>
                  ...
                  <p style={{ color: '#64748b', fontSize: 13.5, marginBottom: 6 }}>
                    Token number assigned
                  </p>
                  <p style={{ color: '#94a3b8', fontSize: 12.5, marginBottom: 28 }}>
                    {waiting + 1} patient{(waiting + 1) !== 1 ? 's' : ''} now in queue
                  </p>
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <span className="badge badge-primary">{walkIns.length} total</span>
            </div>
              ...
              <div style={{ maxHeight: 520, overflowY: 'auto', paddingRight: 4 }}>
                {walkIns.map(w => (
                  <div key={w.walkin_id} style={{
                    ...
                    <div className="token-badge" style={{ fontSize: 11 }}>W{w.token_number}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 13.5 }}>{w.patient_name}</div>
